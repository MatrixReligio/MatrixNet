...
> **狀態:第一階段,開發中。** MatrixNet 是一個正在積極開發的早期專案。架構已確定,核心函式庫以測試優先方式建構,但 App 尚未功能完整,也還沒有穩定版本。介面、指令與 UI 都可能變動。
...
第一階段嚴格 **被動——只觀察,不攔截**。沒有防火牆、不攔截流量、也不解密 HTTPS(後續規劃見 [路線圖](#路線圖))。因為只觀察,MatrixNet 能與你既有的代理、過濾或 VPN 並存,而不互相干擾。
...
- DNS 加值會把觀測到的 IP 反查為主機名稱,並提供一鍵切換,在連線與封包檢視中顯示 **網域名稱或原始 IP**。
...
- **GeoIP 資料庫自動更新**——於背景從每月的 DB-IP 資料集更新,讓國家歸屬隨時間保持準確。
...
| | Little Snitch | Wireshark | **MatrixNet(第一階段)** |
|---|:---:|:---:|:---:|
| 逐 App 連線檢視 | ✅ | ❌ | ✅ |
| 封包層級解析 | ❌ | ✅ | ✅ |
| 每個封包都知道所屬 App | ❌ | ❌ | ✅ |
...
# 4. 建置 / 執行 App
#    (在 Xcode 26 開啟 MatrixNet.xcodeproj,或使用 xcodebuild——待定案)
...
第一階段刻意聚焦於被動監控與分析。後續階段規劃中(尚未實作,亦不保證):

- **防火牆 / 攔截**——可選擇啟用的攔截模式(可能透過 `NEFilterDataProvider`),並對與其他 socket 層過濾器可能的衝突給出明確警告。
- **AI 原生分析**——以自然語言查詢你的流量,自動偵測追蹤器 / 異常 / 隱私外洩。
- **HTTPS 解密(MITM)**——可選擇啟用的 TLS 攔截以做明文檢視。
- 遠端 / 行動擷取、規則引擎,以及更廣的 Wireshark 風格通訊協定支援。
