...
- 全系統、逐 App 的即時連線清單:程序、遠端主機/IP、國家、上下行速率、累計位元組,以及連線生命週期。 連線、歷史與用量檢視預設按應用程式彙整——點擊某個應用程式即可下鑽查看其逐條連線。
- 由核心歸屬的程序擁有者——與 `nettop` 和活動監視器相同的機制——歸屬準確,沒有輪詢競態。
- 由連接埠推斷的 **用戶端/伺服端角色**(這台主機是發起連線,還是接受連線?)。
- **代理與 VPN/通道辨識**——遠端為你設定的或本機代理的連線會被標示,而替其他 App 中繼流量的程序(NetworkExtension 通道)會掛上徽章,讓流量是否被轉送一目了然。 開啟封包擷取時,經代理的連線仍會顯示真實網域與流量(從通道介面讀取),「經由代理」指標按位元組計。
...
### 🖥️ 桌面小工具
- WidgetKit 小工具(小 / 中 / 大)顯示作用中連線數、上下行吞吐、工作階段總量、最忙的 App,以及威脅命中數——就在桌面或通知中心。
- 當 App 視窗在前台時即時更新;在後台,macOS 將第三方小工具的刷新限制為大約每 30 分鐘一次(WidgetKit 每日配額)。要看精確到秒的速率,請看選單列。

### 🧭 選單列與背景執行
- 常駐 **選單列**,即時顯示 ↓/↑ 吞吐,並在你關閉主視窗後持續監控——因此小工具讀取的共享資料即使在 App 處於後台時也保持最新。
...
- **GeoIP 資料庫自動更新**——於背景從每月的 DB-IP 資料集更新,讓國家歸屬隨時間保持準確。它同時涵蓋 **IPv4 與 IPv6** 目標位址,因此地圖與國家指標不會少計 IPv6 流量。 本機代理/通道啟用時,目的地 IP 是合成佔位位址,因此國家會改用真實網域解析得出(見隱私)。
...
- **100% 本機、被動擷取。** 所有封包/連線處理都在你的機器上完成——無遙測、無帳號、無雲端。唯一可能的對外請求是對*經代理*流量的可選 GeoIP 國家解析(預設開啟,透過加密 DNS/DoH),可在設定關閉。
...
MatrixNet 全部在本機處理——沒有遙測、不需帳號、不上雲。唯一可能的對外網路請求,是對經代理流量的可選 GeoIP 國家解析(預設開啟):本機代理隱藏真實位址時,透過加密 DNS(DoH)解析網域以補全國家;可在設定關閉,做到不外送任何資料。擷取內容、歷史與設定只存在你的磁碟上。
