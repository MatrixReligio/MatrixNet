...
> **100% 수동적 — 관찰하되 차단하지 않음.** MatrixNet은 커널 통계와 각 패킷의 복사본만 읽으므로 어떤 프록시·필터·VPN과도 충돌 없이 함께 동작합니다. 방화벽도, 트래픽 가로채기도, HTTPS 복호화도 없습니다.
...
MatrixNet은 엄격히 **수동적 — 관찰하되 차단하지 않음** 입니다. 방화벽도, 트래픽 가로채기도, HTTPS 복호화도 없습니다. 오직 관찰만 하므로, MatrixNet은 이미 사용 중인 프록시·필터·VPN과 다투지 않고 함께 동작합니다.

## 기능

### 🔭 연결 모니터링
- 실시간 **개요 대시보드**: 처리량 차트(최근 1분), 핵심 지표(활성 연결, 세션 합계, 활성 앱, 도달한 국가 수, 위협 연결, 프록시 경유 비율), 프로토콜 구성, 상위 목적지 국가, 강화된 상위 통신 앱 목록.
- 시스템 전체·앱별 실시간 연결 목록: 프로세스, 원격 호스트/IP, 국가, 상하향 속도, 누적 바이트, 연결 수명 주기. 연결·기록·사용량 보기는 기본적으로 앱별로 집계되며, 앱을 클릭하면 개별 연결로 드릴다운할 수 있습니다.
- 커널이 귀속한 프로세스 소유권 — `nettop`과 활성 상태 보기가 쓰는 것과 동일한 메커니즘 — 이라 폴링 경쟁 없이 정확합니다.
- 포트로 추론하는 **클라이언트/서버 역할**(이 호스트가 발신했는지, 연결을 수락했는지).
- **프록시·VPN/터널 인식** — 원격이 설정된/로컬 프록시인 연결에는 표시가, 다른 앱의 트래픽을 중계하는 프로세스(NetworkExtension 터널)에는 배지가 붙어 트래픽 경유 여부가 분명합니다. 패킷 캡처가 켜져 있으면 프록시 경유 연결도 실제 도메인과 트래픽 양(터널 인터페이스에서 읽음)을 표시하며, '프록시 경유' 지표는 바이트 기준입니다.
...
- 현재 기간을 보고·청구·감사를 위해 CSV 또는 JSON으로 **내보내기** 할 수 있습니다.

### 🔬 심층 패킷 분석
- **모든 패킷이 소유 PID를 지니는** 패킷 단위 캡처.
- 가장 중요한 프로토콜의 견고한 해부: **Ethernet, IPv4, IPv6, TCP, UDP, ICMP, DNS, TLS(핸드셰이크 / SNI / 인증서), HTTP/1.1**.
- **앱별 JA4 TLS 클라이언트 핑거프린트** —— 복호화 없이 ClientHello에서 각 앱의 TLS 스택(브라우저 엔진 / Go / curl / 의심스러운 라이브러리)을 수동적으로 추론. TLS 계층과 연결 인스펙터에 표시되며 인식된 스택은 라벨이 붙습니다.
- **HTTP/3 / QUIC 가시성** —— QUIC Initial을 수동적으로 복호화(RFC 9001의 공개 DCID 파생 키, 비밀 불필요·MITM 없음)하여 각 HTTP/3 연결의 SNI·ALPN·버전을 읽고 QUIC JA4를 계산. 모두 앱별.
- **앱별 네트워크 품질** —— 각 TCP 연결의 핸드셰이크 RTT·재전송·연결 설정 시간을 수동적으로 측정해 연결 인스펙터에 표시(캡처 시에만·프로브 미전송).
- **앱별 암호화 DNS** —— 어떤 앱이 평문 DNS를 쓰는지, DoT·DoQ·DoH(리졸버 표시)인지를 5-튜플과 호스트명으로 분류. 패킷 캡처 불필요.
- **앱별 활동 타임라인** —— 저장된 사용량으로 각 앱이 언제 활동했는지(시간별·일별)를 히트 스트립으로 표시해 백그라운드·야간 활동이 드러납니다.
...
### 🖥️ 데스크탑 위젯
- WidgetKit 위젯(소 / 중 / 대)이 활성 연결 수, 상하향 처리량, 세션 합계, 상위 통신 앱, 위협 적중 수를 데스크탑이나 알림 센터에 표시.
- 앱 창이 포그라운드에 있을 때는 실시간으로 업데이트됩니다. 백그라운드에서는 macOS가 서드파티 위젯 새로 고침을 약 30분에 한 번으로 제한합니다(WidgetKit 일일 예산). 초 단위 속도는 메뉴 막대에서 확인하세요.

### 🧭 메뉴 막대 및 백그라운드
- **메뉴 막대**에 상주하며 실시간 ↓/↑ 처리량을 표시하고, 메인 창을 닫아도 모니터링을 계속해, 위젯이 읽는 공유 데이터가 앱이 백그라운드에 있어도 최신으로 유지됩니다.
...
- **GeoIP 데이터베이스 자동 갱신** — 매월 DB-IP 데이터셋에서 백그라운드로 갱신되어 국가 귀속이 시간이 지나도 정확하게 유지됩니다. **IPv4와 IPv6** 목적지를 모두 포함하므로 지도와 국가별 지표가 IPv6 트래픽을 적게 집계하지 않습니다. 로컬 프록시/터널이 활성화되면 목적지 IP는 합성 자리표시자이므로 국가는 실제 도메인에서 확인합니다(개인정보 보호 참조).
...
- **100% 로컬·수동 캡처.** 모든 패킷/연결 처리는 당신의 기기에서 이뤄집니다 — 텔레메트리·계정·클라우드 없음. 유일한 외부 요청은 *프록시 경유* 플로우의 선택적 GeoIP 국가 조회(기본 켜짐, 암호화 DNS/DoH)이며 설정에서 끌 수 있습니다.
...
MatrixNet은 모든 것을 로컬에서 처리합니다 — 텔레메트리, 계정, 클라우드가 없습니다. 유일하게 발생할 수 있는 외부 요청은 프록시 경유 플로우의 국가를 보완하는 선택적 GeoIP 조회(기본 켜짐)입니다. 로컬 프록시가 실제 주소를 가릴 때 암호화 DNS(DoH)로 도메인을 확인해 국가를 찾습니다. 설정에서 끄면 어떤 데이터도 기기를 벗어나지 않습니다. 캡처, 기록, 설정은 오직 당신의 디스크에만 존재합니다.

## 버전 관리

MatrixNet은 [유의적 버전](https://semver.org/lang/ko/)을 따릅니다: **주.부.수**.

- **주(MAJOR)** — 호환되지 않는 변경 또는 앱 정체성의 근본적 전환.
- **부(MINOR)** — 하위 호환되는 새 기능.
- **수(PATCH)** — 하위 호환되는 버그 수정.

모든 릴리스는 공증되어 앱 내 업데이트로 배포됩니다. 버전별 변경 사항은 [CHANGELOG](./CHANGELOG.md)를 참고하세요.
