...
- **代理与 VPN/隧道识别** —— 远端为你配置的或本地代理的连接会被标记,而为其他 App 中继流量的进程(NetworkExtension 隧道)会打上徽章,让流量是否被转发一目了然。
...
### 🖥️ 桌面 Widget
- WidgetKit 小组件(小 / 中 / 大尺寸)在桌面或通知中心实时显示活动连接数、上下行吞吐、会话累计流量、流量最高的 App,以及威胁命中数。

### 🧭 菜单栏与后台
- 常驻**菜单栏**,实时显示 ↓/↑ 吞吐,并在你关闭主窗口后持续监控 —— 让桌面小组件永不过时。
...
- **GeoIP 数据库自动后台更新**,数据源为按月发布的 DB-IP 数据集,使国家/地区归因长期保持准确。它同时覆盖 **IPv4 与 IPv6** 目标地址,因此地图与国家/地区指标不会少计 IPv6 流量。
