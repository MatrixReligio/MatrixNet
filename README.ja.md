...
> **100% パッシブ——観察するだけで、ブロックはしない。** MatrixNet はカーネルの統計と各パケットのコピーだけを読み取るため、あらゆるプロキシ・フィルタ・VPN と衝突せずに共存します。ファイアウォールも、トラフィック傍受も、HTTPS 復号もありません。
...
MatrixNet は厳密に **パッシブ——観察するだけで、ブロックはしない** です。ファイアウォールも、トラフィックの傍受も、HTTPS の復号もありません。観察に徹するため、MatrixNet はすでに使っているプロキシ・フィルタ・VPN と衝突せずに共存します。

## 機能

### 🔭 接続モニタリング
- ライブの **概要ダッシュボード**：スループットチャート（直近 1 分）、主要指標（アクティブ接続数、セッション合計、アクティブなアプリ数、到達した国数、脅威接続数、プロキシ経由の割合）、プロトコル構成、宛先国トップ、強化版トップトーカー一覧。
- システム全体・アプリ別のライブ接続一覧：プロセス、リモートホスト/IP、国、上下レート、累積バイト、接続のライフサイクル。 接続・履歴・使用状況の各ビューは既定でアプリ別に集約され、アプリをクリックすると個々の接続にドリルダウンできます。
- カーネルが帰属するプロセス所有者——`nettop` やアクティビティモニタと同じ仕組み——なのでポーリング競合なしに正確に帰属します。
- ポートから推定する **クライアント/サーバーの役割**（このホストが発信したのか、接続を受け入れたのか）。
- **プロキシ・VPN/トンネルの認識**——リモートが設定済み/ローカルのプロキシである接続には印が付き、他アプリのトラフィックを中継するプロセス（NetworkExtension トンネル）にはバッジが付くため、経由されているかが一目で分かります。 パケットキャプチャ有効時、プロキシ経由の接続でも実ドメインと通信量（トンネルインターフェイスから取得）が表示され、「プロキシ経由」指標はバイト基準になります。
- **脅威 IP のフラグ付け**——公開された脅威インテリジェンスのブロックリストに載るリモートアドレスに ⚠️ バッジ（助言のみ——MatrixNet はラベル付けするだけでブロックはしません）。
- **新しい接続先（「phoning home」）通知**——任意・非ブロック:既知のアプリがこれまで接続したことのない国・地域に初めて接続したときに通知します。アプリごとの学習期間とレート制限で静かに——アウトバウンドファイアウォールの洞察を、ブロックも通知洪水もなしで。
...
- 現在の期間を CSV または JSON に**エクスポート**——レポート・請求・監査に。

### 🔬 ディープパケット解析
- **すべてのパケットが所有 PID を保持する**パケット単位のキャプチャ。
- 最も重要なプロトコルの堅牢な解析：**Ethernet、IPv4、IPv6、TCP、UDP、ICMP、DNS、TLS（ハンドシェイク / SNI / 証明書）、HTTP/1.1**。
- **アプリ別の JA4 TLS クライアントフィンガープリント** —— 復号せずに ClientHello から各アプリの TLS スタック(ブラウザエンジン / Go / curl / 不審なライブラリ)を受動的に推定。TLS レイヤーと接続インスペクタに表示され、既知のスタックはラベル付けされます。
- **HTTP/3 / QUIC の可視化** —— QUIC Initial を受動的に復号(RFC 9001 の公開 DCID 派生鍵、機密不要・MITM なし)して各 HTTP/3 接続の SNI・ALPN・バージョンを読み、QUIC JA4 を算出。すべてアプリ別。
- **アプリ別ネットワーク品質** —— 各接続の TCP ハンドシェイク RTT・再送・接続確立時間を受動的に計測し、接続インスペクタに表示(キャプチャ時のみ・プローブ送信なし)。
- **アプリ別暗号化 DNS** —— どのアプリが平文 DNS のままか、DoT・DoQ・DoH(リゾルバ名付き)かを 5 タプルとホスト名から判定。パケットキャプチャ不要。
- **アプリ別アクティビティタイムライン** —— 保存済みの使用量から、各アプリがいつ活動していたか(時間別・日別)をヒートストリップで表示。バックグラウンド/夜間の活動が一目で分かる。
...
### 🖥️ デスクトップウィジェット
- WidgetKit ウィジェット（小 / 中 / 大）が、アクティブ接続数、上下スループット、セッション合計、上位アプリ、脅威ヒット数を、デスクトップや通知センターに表示。
- アプリのウィンドウがフォアグラウンドにあるときはライブで更新。バックグラウンドでは、macOS はサードパーティウィジェットの更新を約30分ごとに制限します(WidgetKit の1日あたりの予算)。秒単位のレートはメニューバーで確認してください。

### 🧭 メニューバーとバックグラウンド
- **メニューバー**に常駐し、ライブの ↓/↑ スループットを表示。メインウィンドウを閉じても監視を続けるため、ウィジェットが読み取る共有データはアプリがバックグラウンドにあっても最新に保たれます。
...
- **GeoIP データベースは自動更新**——毎月の DB-IP データセットからバックグラウンドで更新され、国の帰属が時間とともに正確に保たれます。**IPv4 と IPv6** の両方の宛先をカバーするため、マップや国別メトリクスが IPv6 トラフィックを過小カウントすることはありません。 ローカルプロキシ/トンネルが有効な場合、宛先 IP は合成プレースホルダーのため、国は実ドメインから解決されます（プライバシー参照）。
...
- **100% ローカル・パッシブ取得。** すべてのパケット/接続処理はあなたのマシン上で行われます——テレメトリ・アカウント・クラウドなし。唯一の外部リクエストは*プロキシ経由*フローの任意の GeoIP 国解決(既定で有効、暗号化 DNS/DoH)で、設定でオフにできます。
...
MatrixNet はすべてをローカルで処理します——テレメトリもアカウントもクラウドもありません。唯一発生しうる外部通信は、プロキシ経由フローの国を補完する任意の GeoIP 解決(既定で有効)です。ローカルプロキシが実アドレスを隠す場合、暗号化 DNS（DoH）でドメインを解決して国を求めます。設定でオフにすればデータは一切外部に出ません。キャプチャ・履歴・設定はあなたのディスク上にのみ存在します。

## バージョニング

MatrixNet は [セマンティック バージョニング](https://semver.org/lang/ja/) に従います：**メジャー.マイナー.パッチ**。

- **メジャー（MAJOR）**——互換性のない変更、またはアプリの位置づけの根本的な転換。
- **マイナー（MINOR）**——後方互換のある新機能。
- **パッチ（PATCH）**——後方互換のあるバグ修正。

すべてのリリースは公証され、アプリ内アップデートで配信されます。各バージョンの変更点は [CHANGELOG](./CHANGELOG.md) を参照してください。
